...
lives in `lib/stripe.ts`, `lib/billing/subscriptions.ts`, and the API routes under
`app/api/stripe/`.
...
STRIPE_PRICE_ID_MONTHLY=price_...       # Server-side only (monthly plan)
STRIPE_PRICE_ID_YEARLY=price_...        # Server-side only (annual plan)
NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=pk_live_...
```

## Checkout Flow

1. Owner selects Monthly (€49/mo) or Annual (€470/yr) on `/admin/billing`
2. `BillingCheckoutButton` POSTs to `/api/stripe/checkout?plan=monthly|annual`
3. Route creates a Stripe Checkout Session (`mode: 'subscription'`) using the
   `STRIPE_PRICE_ID_MONTHLY` or `STRIPE_PRICE_ID_YEARLY` price ID
4. Owner is redirected to Stripe; on success returns to `/admin/dashboard?session_id={id}`
5. Stripe sends a `checkout.session.completed` webhook → `lib/billing/subscriptions.ts`
   updates the `restaurants` table and logs to `billing_ledger`
...
|---|---|---|
| Monthly | `STRIPE_PRICE_ID_MONTHLY` | €49/mo |
| Annual | `STRIPE_PRICE_ID_YEARLY` | €470/yr |
