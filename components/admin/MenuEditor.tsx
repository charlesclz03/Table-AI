...
import { MenuPhotoUpload } from '@/components/admin/MenuPhotoUpload'
...
export function MenuEditor({ initialItems }: MenuEditorProps) {
  ...
  const [activeMode, setActiveMode] = useState<EditorMode>('manual')
  ...
  function handleParsedMenuSaved(savedItems: AdminMenuItem[]) {
    setItems(savedItems)
    setStatus('Menu imported from photo and saved to Supabase.')
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <div className="rounded-[28px] border border-white/10 bg-white/6 p-5 backdrop-blur">
          <p className="text-[11px] uppercase tracking-[0.3em] text-amber-200/70">
            {activeMode === 'manual' ? 'Editing Notes' : 'AI Import'}
          </p>
          {activeMode === 'manual' ? (
            <>
              <ul className="mt-4 space-y-3 text-sm leading-6 text-white/70">
                ...
                <li>
                  Vegetarian and vegan badges feed concierge recommendations.
                </li>
              </ul>
            ...
            <>
              <ul className="mt-4 space-y-3 text-sm leading-6 text-white/70">
                <li>Upload one or many menu pages as images or a PDF.</li>
                <li>AI extracts dishes, prices, categories, and allergens.</li>
      ...
      {activeMode === 'manual' ? (
        ...
        <MenuPhotoUpload onSaved={handleParsedMenuSaved} />
      )}
    </div>
  )
}
