# Competitive Analysis — Table IA
**Date:** 2026-04-02

---

## Competitors

### Hostie AI
- **Price:** $399-599/month
- **Focus:** Voice AI for restaurants
- **Languages:** 57+
- **Target:** US market
- **Weakness:** Expensive, no text/chat

### ActiveMenus
- **Price:** ~$99/month (estimated)
- **Focus:** Voice ordering
- **Languages:** 57+
- **Claim:** 10K+ restaurants
- **Weakness:** Voice-only, US-focused

### MenuQ
- **Price:** Unknown
- **Focus:** 3D QR menus + voice AI
- **Features:** Multilingual QR integration
- **Weakness:** Complex, expensive

### QRCodeKIT
- **Price:** Unknown
- **Focus:** Conversational QR menus
- **Launch:** 2 weeks ago
- **Weakness:** New entrant, unproven

### Amora AI
- **Price:** Unknown
- **Focus:** Menu image → mobile ordering site in 60 seconds
- **Weakness:** Ordering-focused, not concierge

### AIDA
- **Price:** Unknown
- **Focus:** AI dining assistant + QR menus
- **Features:** Personalized recommendations
- **Weakness:** Generic, no voice focus

### Slang AI
- **Price:** $399-599/month
- **Focus:** Voice AI for restaurants
- **Target:** US enterprise
- **Weakness:** Expensive, enterprise-focused

---

## Our Advantages

| Competitor | Price | Voice | Text | Self-Serve | Lisbon Focus |
|---|---|---|---|---|---|
| Hostie AI | $399-599 | ✅ | ❌ | Unknown | ❌ |
| ActiveMenus | ~$99 | ✅ | ❌ | Unknown | ❌ |
| MenuQ | $$$ | ✅ | ✅ | Unknown | ❌ |
| QRCodeKIT | $$ | ✅ | ✅ | Yes | ❌ |
| **Table IA** | **€299 setup + €49/mo** | **✅** | **✅** | **Yes** | **Lisbon** |

**Key differentiators:**
1. **Much cheaper ongoing** than US competitors, while charging enough upfront to fund installation
2. **Voice + text** (most are voice-only)
3. **Self-service onboarding** (quiz + menu photo)
4. **Pre-built demo** sales method
5. **Lisbon market focus** (local, relationship-first)
6. **Real conversations** not just ordering

---

## Threats

1. **QRCodeKIT just launched** — AI chat for QR menus is exactly what we're doing
2. **Amora AI** — 60-second setup is faster than our 5-min quiz
3. **Hostie AI** — well-funded, 57 languages, established

---

## Opportunities

1. **No direct competitor in Portugal/Lisbon** — market is underserved
2. **Relationship-first approach** — Portuguese restaurants respond to personal sales
3. **Demo-first sales** — most competitors just have websites
4. **Founder-installed offer** — setup fee matches the real onboarding work and feels more credible than ultra-cheap SaaS
